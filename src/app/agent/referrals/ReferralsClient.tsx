...
export default function ReferralsClient({ profile, agentProfile, initialReferrals }: { profile: any, agentProfile: any, initialReferrals: any[] }) {
  const [referrals, setReferrals] = useState(initialReferrals);
  const [showInvite, setShowInvite] = useState(false);
  const [inviteForm, setInviteForm] = useState({ name: "", email: "", university: "", note: "" });
  const [inviteSent, setInviteSent] = useState(false);
  
  const referralCode = agentProfile.referral_code || "GEP-2026-DM";
  ...
  const statusColor = (s: string) => {
    if (s === "repaying" || s === "disbursed" || s === "paid_off" || s === "approved") return "#10B981";
          ...
          <tbody>
            {referrals.map((ref) => (
              <tr key={ref.id}>
                ...
                <td style={{ ...s.td, color: "#10B981" }}>{ref.loan_amount ? formatCAD(Number(ref.loan_amount)) : "---"}</td>
              </tr>
            ))}
            {referrals.length === 0 && (
              <tr><td colSpan={5} style={{...s.td, textAlign: "center"}}>No referrals yet. Share your link to get started!</td></tr>
            )}
          </tbody>
        </table>
      </div>
