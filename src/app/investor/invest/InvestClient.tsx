...
export default function InvestClient({ profile }: { profile: any }) {
  ...
  // Use a Server Action for actual investment in a later phase. 
  // For now we keep the UI flow.
    ...
    <DashboardLayout role="investor" userName={profile.full_name || "Investor"}>
      ...
      {mode === "split" && (
        <div style={s.splitCard}>
          ...
          <button style={{ ...s.investBtn, opacity: agreed ? 1 : 0.5 }} disabled={!agreed} onClick={() => setShowConfirm(true)}>Confirm Split Investment →</button>
        </div>
      )}
              ...
              <>
                ...
                <div style={s.confirmCard}>
                  {mode === "single" ? (
                    ...
                    <>
                      <div style={s.confirmRow}><span>Type</span><strong>Split Allocation</strong></div>
                      <div style={s.confirmRow}><span>Senior ({splitSenior}%)</span><strong style={{ color: "#10B981" }}>{formatCAD(seniorAmount)}</strong></div>
                      <div style={s.confirmRow}><span>Junior ({100 - splitSenior}%)</span><strong style={{ color: "#F59E0B" }}>{formatCAD(juniorAmount)}</strong></div>
                      <div style={s.confirmRow}><span>Total Amount</span><strong style={{ color: "#F9FAFB" }}>{formatCAD(totalSplitAmount)}</strong></div>
                      <div style={s.confirmRow}><span>Blended Yield</span><strong style={{ color: "#14B8A6" }}>~{formatCAD(splitYield)}/yr</strong></div>
                    </>
                  )}
                </div>
              ...
              <div style={{ textAlign: "center" as const, padding: "20px 0" }}>
                ...
                <p style={{ fontSize: "14px", color: "#6B7280", lineHeight: 1.7, margin: "12px 0" }}>
                  {mode === "single" ? (
                    ...
                    <>{formatCAD(totalSplitAmount)} has been successfully split across both tranches.</>
                  )}
                </p>
