...
export default function InvoicesClient({ profile, invoices }: { profile: any, invoices: any[] }) {
          ...
          <tbody>
            {filtered.map((inv) => (
              <tr key={inv.id}>
                ...
                <td style={{ ...s.td, fontWeight: 700, color: "#F9FAFB" }}>{formatCAD(Number(inv.tuition_amount))}</td>
                <td style={s.td}>{inv.edukard_loan_id ? <span style={s.matchedText}>✓ {inv.edukard_loan_id.slice(0, 8)}...</span> : <span style={s.unmatchedText}>None</span>}</td>
                <td style={s.td}>
                  <span style={{
                    ...s.badge,
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={6} style={{...s.td, textAlign: "center"}}>No invoices found</td>
              </tr>
            )}
          </tbody>
        </table>
