...
/**
 * Fallback onboarding UI shown when Sumsub is NOT configured. Demonstrates the
 * verification steps without calling a provider. When SUMSUB_APP_TOKEN /
 * SUMSUB_SECRET_KEY are set, the page renders the real WebSDK instead.
 */
  ...
  return (
    <>
      ...
      <div style={s.noticeBox}>
        <strong style={{ color: "#F59E0B" }}>Demo mode</strong>
        <span style={{ color: "#D1D5DB", fontSize: 13 }}> — add SUMSUB_APP_TOKEN &amp; SUMSUB_SECRET_KEY to launch the live Sumsub identity check.</span>
      </div>
            ...
            {currentStep === 1 && (
              <div style={s.formFields}>
                <div style={s.fieldRow}>
                  <div style={s.field}><label style={s.label}>First Name</label><input style={s.input} placeholder="First name" /></div>
                  <div style={s.field}><label style={s.label}>Last Name</label><input style={s.input} placeholder="Last name" /></div>
                </div>
                <div style={s.fieldRow}>
                  <div style={s.field}><label style={s.label}>Date of Birth</label><input style={s.input} type="date" /></div>
                  <div style={s.field}><label style={s.label}>Nationality</label><input style={s.input} placeholder="Nationality" /></div>
                </div>
                <div style={s.field}><label style={s.label}>Canadian Address</label><input style={s.input} placeholder="123 College St, Toronto, ON M5T 1R4" /></div>
              </div>
            )}
...
const s: Record<string, React.CSSProperties> = {
  ...
  noticeBox: { background: "rgba(245,158,11,0.08)", border: "1px solid rgba(245,158,11,0.15)", borderRadius: "12px", padding: "12px 16px", marginBottom: "20px" },
  ...
  stepItem: { display: "flex", alignItems: "center", gap: "12px", padding: "14px 16px", borderRadius: "12px", border: "1px solid transparent", background: "transparent", cursor: "pointer", textAlign: "left", width: "100%" },
  ...
  uploadBox: { border: "2px dashed rgba(75,85,99,0.3)", borderRadius: "14px", padding: "32px 24px", textAlign: "center", background: "rgba(31,41,55,0.2)" },
  ...
  cameraBox: { textAlign: "center", maxWidth: "400px" },
  ...
  successCard: { background: "rgba(17,24,39,0.6)", backdropFilter: "blur(12px)", border: "1px solid rgba(16,185,129,0.2)", borderRadius: "14px", padding: "48px", textAlign: "center", marginTop: "20px" },
