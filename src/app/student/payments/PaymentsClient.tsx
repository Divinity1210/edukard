...
export default function PaymentsClient({ profile, loan, schedule }: { profile: any, loan: any, schedule: any[] }) {
  const [calendarMonth, setCalendarMonth] = useState(4); // May for demo, could be new Date().getMonth()
  ...
  return (
    <DashboardLayout role="student" userName={profile.full_name || "Student"}>
      ...
      {!loan ? (
        <div style={s.section}>
          <div style={s.padCard}>
            <p style={{ color: "#9CA3AF" }}>You do not have an active loan to make payments on.</p>
          </div>
        </div>
      ) : (
        <>
          ...
          <div style={s.padCard}>
            ...
            <div style={s.padDetails}>
              ...
              <div style={s.padDetail}><span style={s.pdLabel}>Next Debit</span><span style={s.pdValue}>{nextPayment ? formatDate(nextPayment.due_date) : "N/A"}</span></div>
              <div style={s.padDetail}><span style={s.pdLabel}>Mandate Signed</span><span style={s.pdValue}>{formatDate(loan.created_at)}</span></div>
            </div>
          </div>
          ...
          {schedule.length > 0 && (
            <div style={s.section}>
              ...
              <div style={s.tableCard}>
                <table style={s.table}>
                  <thead><tr>{["#", "Due Date", "Principal", "Interest", "Total", "Status"].map(h => <th key={h} style={s.th}>{h}</th>)}</tr></thead>
                  <tbody>
                    {schedule.map((row) => (
                      <tr key={row.id}>
