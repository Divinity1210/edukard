...
import { logCommunication, markDefault } from "@/lib/actions/admin";

type Comm = { id: string; channel: string; subject: string; message: string; sent_at: string; sent_by: string | null; status: string };
type Account = {
  loan_id: string;
  borrower: string;
  university: string;
...
export default function CollectionsClient({ accounts, totalComms }: { accounts: Account[]; totalComms: number }) {
  ...
  function sendComm(loanId: string, channel: string, subject: string, message: string) {
    setError(null);
    ...
    startTransition(async () => {
      const r = await logCommunication(fd);
  ...
  function confirmDefault() {
    if (!defaultModal || !defaultReason) return;
    setError(null);
    const fd = new FormData();
    fd.set("loanId", defaultModal);
    fd.set("reason", defaultReason);
    fd.set("notes", defaultNotes);
    startTransition(async () => {
      const r = await markDefault(fd);
      if (r?.error) {
        setError(r.error);
      } else {
  ...
  return (
    <>
      ...
      <div style={s.summaryGrid}>
        ...
        <div style={s.summaryCard}><span style={{ ...s.summaryValue, color: "#6B7280" }}>{totalComms}</span><span style={s.summaryLabel}>Communications Sent</span></div>
      </div>

      {accounts.length === 0 && (
        <div style={s.card}><p style={{ color: "#9CA3AF" }}>No delinquent accounts. 🎉</p></div>
      )}

      <div style={s.list}>
        {accounts.map((acc) => {
          const isExpanded = expanded === acc.loan_id;
          const accComms = acc.comms;
          ...
          return (
            <div key={acc.loan_id} style={{ ...s.card, borderLeft: `4px solid ${statusColors[acc.status]}` }}>
              ...
              <div style={s.actions}>
                <button disabled={pending} style={s.actionBtn} onClick={() => sendComm(acc.loan_id, "email", "Payment Reminder", "This is a reminder that your scheduled tuition repayment is overdue. Please make a payment to avoid additional fees.")}>📧 Send Reminder</button>
                <button disabled={pending} style={s.actionBtn} onClick={() => sendComm(acc.loan_id, "phone", "Collection Call", "Outbound collection call logged by admin.")}>📞 Log Call</button>
                <button
                  disabled={pending}
                  onClick={() => { setDefaultModal(acc.loan_id); setDefaultReason(""); setDefaultNotes(""); }}
        ...
        <div style={s.modalOverlay} onClick={() => setDefaultModal(null)}>
          <div style={s.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={s.modalBody}>
              ...
              <textarea value={defaultNotes} onChange={(e) => setDefaultNotes(e.target.value)} placeholder="Document any relevant context for the default record..." style={s.textarea} rows={3} />

              <div style={s.warningBox}>
                <strong style={{ color: "#EF4444" }}>⚠️ This action is irreversible</strong>
                <p style={{ fontSize: "13px", color: "#D1D5DB", lineHeight: 1.6, marginTop: "6px" }}>Issuing a default notice will:</p>
            ...
            <div style={s.modalFooter}>
              ...
              <button style={{ ...s.modalConfirmDefault, opacity: defaultReason && !pending ? 1 : 0.5 }} disabled={!defaultReason || pending} onClick={confirmDefault}>
                {pending ? "Processing..." : "⚠️ Confirm Default Notice"}
              </button>
            </div>
          </div>
        </div>
